...
// *************************************************************************
    ...
    if(isset($_POST['person_receives'])){
        ...
        if (empty($errors)) {
            ...
            $db->bind(':order_id',  $id);
            $db->bind(':user_id',  $_SESSION['userid']);
            $db->bind(':action','has delivered the shipment'); 
            ...
            $db->bind(':notification_description','The shipment has been delivered, please check it');           
            ...
            $subject="Shipment has been delivered, tracking number $fullshipment";
            $status_courier_deliver='Delivered';



            $email_template =getEmailTemplates(18);
                                      ...
                                      ->create("whatsapp:".$phone_sender, // to 
                                               array( 
                                                   "from" => "whatsapp:".$core->twilio_number,       
                                                    "body" => "a new shipment has been delivered, *Tracking #$fullshipment* Follow up on your package by entering the following link and you will have detailed information on the status of your packages $app_url"
                                               ) 
                                      );
                                                ...
                                                <div class="form-group col-md-6">
                                                    ...
                                                    <div class="input-group mb-3">                                              
                                                        ...
                                                        <?php foreach ($driverrow as $row):?>
                                                            <option value="<?php echo $row->id; ?>" <?php if($row->id==$_SESSION['userid']){ echo 'selected';}?> ><?php echo $row->fname.' '.$row->lname; ?></option>
                                                        <?php endforeach;?>
                                                        </select>
                                                    </div>
                                                </div> 
