		...
		<?php
			...
			if (isset($_GET['data'])) {
			    ...
			    foreach ($data as $key) {
			    	...
			    	$data_key= getCourierPrintMultiple($key);			
			    ...
			    // var_dump($row->order_id);
					...
					$db->query("SELECT SUM(order_item_length) as total0 from add_order_item where order_id='".$row->order_id."'");
					$db->execute();
					$row0 =$db->registro();
					...
					$db->query("SELECT SUM(order_item_width) as total1 from add_order_item where order_id='".$row->order_id."'");
					$db->execute();
					$row1 =$db->registro();
					...
					$db->query("SELECT SUM(order_item_height) as total2 from add_order_item where order_id='".$row->order_id."'");
					$db->execute();
					$row2 =$db->registro();

					$rw_add2= $row2->total2;

					$db->query("SELECT SUM(order_item_height) as total3 from add_order_item where order_id='".$row->order_id."'");
					$db->execute();
					$row3 =$db->registro();
					...
					$db->query("SELECT * FROM add_order_item WHERE order_id='".$row->order_id."'"); 
				...
				<div class="container_etiqueta" style="min-height: 540px; max-height: 540px; min-width: 420px; max-width: 420px;">
					<div class="print_ticket_zebra" style="max-height: 110px; min-height: 110px;">
						<div style="width: 30%;line-height:110px;margin:0px auto;text-align:center;">
								...
								<?php echo ($core->logo) ? '<img class="logo" style="vertical-align:middle"  src="assets/'.$core->logo.'"  width="145" height="70" alt="'.$core->site_name.'" />': $core->site_name;?>
						</div>
						<div style="width: 70%;">
					...
					<div class="app_easypack_ticket_zebra" style="padding-top: 3px; padding-bottom: 3px; font-size: 22px; max-height: 18px; min-height: 25px;">
						<strong><?php echo $met_payment->met_payment; ?></strong>
					</div>
					<br>
