...
class LicenseBoxExternalAPI{
	...
	public function __construct(){ 
		$this->product_id = '4904B25B';
		$this->api_url = 'https://url.google.com/';
	...
	public function activate_license($license, $client, $create_lic = true){
		return array('status' => TRUE, 'message' => 'Valid');
		$data_array =  array(
			"product_id"  => $this->product_id,
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["bfded32c7c31782"])){
				$_SESSION["bfded32c7c31782"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["bfded32c7c31782"])){
				$get_data = $this->call_api(
					'POST',
