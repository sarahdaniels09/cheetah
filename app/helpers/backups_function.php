...
function doBackup()
{
    ...
    foreach ($tables_to_backup as $table) {
       ...
       $sql1   = $db->query("SELECT * FROM $table");
       $result = $db->fetch_all();
       $filas  = $db->rowCount();

       $num        = $db->query("SELECT COUNT(COLUMN_NAME) AS columna FROM information_schema.columns WHERE table_schema = '$db_name' AND table_name = '$table'");
       $num_fields = $db->registro();
    ...
    ;
    $db->query('UPDATE settings SET  backup =:backup');
                    
    $db->bind(':backup', $file_update);     

    $db->execute(); 

    redirect_to("backup.php?do=backup&backupok=1"); 
