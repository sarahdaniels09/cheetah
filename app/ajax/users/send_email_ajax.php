      ...
      if (empty($errors)) {
        ...
        switch ($to) {
          case "all":
            ...
            $db->query("SELECT email, CONCAT(fname,' ',lname) as name FROM users  WHERE id != 1");

            $db->execute();        

            $userrow= $db->registros();
          ...
          //SENDMAIL PHP
            ...
            if ($core->mailer=='PHP') {
               ...
               /*SIGUE RECOLECTANDO DATOS PARA FUNCION MAIL*/
                ...
                $subject=$subject;      
                $mail_send= mail(email_users_notifications($array),$subject,$message,$header); 
                ...
                $message = $body;
                $destinatario= email_users_notifications($array);
          ...
          $db->query("SELECT email, CONCAT(fname,' ',lname) as name FROM users WHERE newsletter = '1' AND id != 1");
        ...
        if ($core->mailer=='PHP') {  
           ...
           /*SIGUE RECOLECTANDO DATOS PARA FUNCION MAIL*/
            ...
            $db->query("SELECT email, CONCAT(fname,' ',lname) as name FROM users WHERE email LIKE '%" . trim($to) . "%'");
