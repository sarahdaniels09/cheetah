...
// *************************************************************************
	...
	$db->query("
		...
		SELECT b.user_id,  b.shipping_type,  a.id_notifi_user, b.notification_description, b.notification_date , b.order_id , a.notification_status, a.notification_read, b.notification_id

		FROM notifications_users as a

		INNER JOIN notifications as b ON a.notification_id = b.notification_id
    ...
    <ul class="list-style-none" >
        ...
        if($rowCount>0){
        	...
        	foreach ($data as $key ) {
               ...
               switch ($key->shipping_type) {
                   ...
                   default:
                       # code...
                    $href='users_edit.php?user='.$key->user_id;
        ...
        <?php 
         		...
         		if( $key->notification_status==0){ 
					...
					// $(document).prop('title', fullName+ 'AJfffdasdas Te ha enviado un mensaje');

					</script>" ;

					// var_dump($_SESSION['userid']);
					updateNotificationStatus($_SESSION['userid'], $key->notification_id);
        ...
        <li>
            <a class="nav-link text-center m-b-5" href="notifications_list.php"> <strong style="color: black">Check all notifications</strong> <i class="fa fa-angle-right"></i> </a>
        </li>
    </ul>
